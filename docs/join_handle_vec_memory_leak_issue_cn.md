...
当前 SLA 类型检查允许 `Vec<JoinHandle<i32>>`，但生成 SA 后端在函数退出时报 `MemoryLeak`。即使先从尾部读取 handle、`join()` 后再 `pop()`，仍有 active register 残留。这阻塞了下游直接实现“动态 N 个 worker handle 存入 Vec 后统一 join”的 executor 路线。
