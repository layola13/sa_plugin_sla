...
- [done] Converged array-typed struct field ABI through shared lowering rules instead of a SAB-only layout branch.
  ...
  - Current no-fallback `tests/test_unit_*.sla` sweep is now 38/64 passing. Remaining 26 failures are broader direct-SAB gaps: async, blank identifier, borrow-temp release ordering, cell bool, derive/component semantics, dyn borrow args, enum match, nested field compare/len, for-in protocols, impl static methods, Option methods, pkgjson codegen, rc/dyn trait, refcell/smart-pointer paths, sets, nested import contract extern callee resolution, spaceship compare, std import, struct update, trait static dispatch, using static extension, var comprehensive, and Vec index assignment.
  - Feature completion: 100% for array-typed struct field ABI convergence. Broader Y/shared-lowering progress is now approximately 48%; direct SAB fallback-removal progress is now approximately 73% by feature track, with current no-fallback unit-file pass rate at 38/64.
