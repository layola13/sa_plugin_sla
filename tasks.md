...
- [x] **Direct SAB Output Mainline**
  ...
  - [x] Converge direct-SAB control-flow release scoping and struct-operator lowering with the SA-text emitter so both backends agree on a shared corpus.
    ...
    - Current no-fallback `tests/test_unit_*.sla` sweep is 43/64 passing after the later array-field ABI, dyn-borrow plan-consumption, and static dispatch resolved-symbol convergence slices below. Remaining direct-SAB gaps include async/blank identifier/for-in/enum/set/derive semantics shapes, rc/dyn/refcell/smart-pointer paths, import-contract extern callee resolution, struct update, var comprehensive, and Vec index assignment.
    - Progress estimate: 100% for these three direct-SAB divergence fixes; broader Y/shared-lowering progress is approximately 52%, while direct SAB fallback removal is approximately 76% by feature track after the later convergence slices.
  ...
  - [x] Converge inherent and trait static dispatch through the shared resolved-call symbol plan.
    ...
    - Verified in the current tree: no-fallback SAB and SA-text backend for `test_unit_impl_static_methods.sla` and `test_unit_trait_static_dispatch.sla`; full current no-fallback `tests/test_unit_*.sla` sweep is 43/64 passing. This also moved `test_unit_derive_component.sla` and `test_unit_using_static_extension.sla` into the passing set.
    - Progress estimate: 100% for static dispatch resolved-symbol convergence; broader Y/shared-lowering progress is approximately 52%, and direct SAB fallback removal is approximately 76% by feature track.
  ...
  - [ ] Remove the remaining in-memory SA-compatible fallback from the normal SAB path by replacing std/macro/closure gaps with generic direct SAB lowering or a generic SAB macro representation, not library-name special cases.
    - Progress estimate: approximately 76% overall direct SAB fallback removal; next priority is direct SAB consumption of the remaining shared call/materialization plan kinds and result-destination planning so both emitters reuse lowering decisions.
