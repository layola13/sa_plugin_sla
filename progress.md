...
- [draft] Next active slice is SCI extern/export declaration ordering hardening plus plugin-side fragment cleanup audit.
  - Current verified baseline: full dev-mode direct-SAB no-fallback sweep is 70/70 passing; Y/shared-lowering is approximately 93%; direct SAB fallback-removal is 100% for the tracked unit corpus.
  ...
  - SCI boundary note: generic std-macro fragment naming belongs in `sci` flatten/encode. Embedded symbol-token/call-body remap is now fixed in SCI for `appendFlattenFragment`; extern/export ordering and plugin-side fragment cleanup remain open. Plugin-side `debug()` structured lowering is retained because it is a proper direct SAB structured path, not a fixture hack.
  - Active slice target: preserve imported extern/export declarations in verifier-safe order, then audit whether `sa_plugin_sla` fragment remap/placeholder handling can consume SCI-owned behavior instead of maintaining a parallel semantic implementation. Current progress: 55% for this slice; owner path identified in SCI verifier predecoded metadata and `sa_plugin_sla` filtered std-dep injection. Completed sub-slice: filtered std-dep injection now includes same-module direct-call dependencies in original decoded-module order and de-duplicates repeated selected helper names, fixing the HashSet/BTreeSet helper declaration/body coherence path without adding fixture-specific macro rewrites. Broader SCI boundary convergence is about 65% after the embedded token-remap subfeature plus the filtered-dep closure sub-slice.
