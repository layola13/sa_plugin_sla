...
- [done] Inherent and trait static dispatch now converge through the shared resolved-call symbol plan.
  ...
  - Verified no-fallback direct SAB for `tests/test_unit_impl_static_methods.sla` and `tests/test_unit_trait_static_dispatch.sla`, plus SA-text backend runs for both fixtures. The full current no-fallback `tests/test_unit_*.sla` sweep is now 43/64 passing; the same change also moves `test_unit_derive_component.sla` and `test_unit_using_static_extension.sla` into the passing set.
  - Remaining 21 failures are broader direct-SAB gaps: async, blank identifier, borrow-temp release ordering, cell bool, derive semantics, enum match, nested field compare/len, for-in protocols, Option methods, pkgjson codegen, rc/dyn trait, refcell/smart-pointer paths, sets, nested import contract extern callee resolution, spaceship compare, std import, struct update, var comprehensive, and Vec index assignment.
  - Feature completion: 100% for static dispatch resolved-symbol convergence. Broader Y/shared-lowering progress is now approximately 52%; direct SAB fallback-removal progress is now approximately 76% by feature track, with current no-fallback unit-file pass rate at 43/64.
