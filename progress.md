...
- [draft] Next active slice is SCI extern/export declaration ordering hardening plus plugin-side fragment cleanup audit.
  ...
  - Active slice target: preserve imported extern/export declarations in verifier-safe order, then audit whether `sa_plugin_sla` fragment remap/placeholder handling can consume SCI-owned behavior instead of maintaining a parallel semantic implementation. Current progress: 25% for this next slice; owner path identified in SCI verifier predecoded metadata and `sa_plugin_sla` filtered std-dep injection. Broader SCI boundary convergence is about 60% after the embedded token-remap subfeature.
