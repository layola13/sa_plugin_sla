...
- [done] Direct SAB dyn-borrow call arguments now consume the shared call-materialization plan.
  ...
  - Current no-fallback `tests/test_unit_*.sla` sweep is now 39/64 passing. Remaining 25 failures are broader direct-SAB gaps: async, blank identifier, borrow-temp release ordering, cell bool, derive/component semantics, enum match, nested field compare/len, for-in protocols, impl static methods, Option methods, pkgjson codegen, rc/dyn trait, refcell/smart-pointer paths, sets, nested import contract extern callee resolution, spaceship compare, std import, struct update, trait static dispatch, using static extension, var comprehensive, and Vec index assignment.
  - Feature completion: 100% for direct SAB dyn-borrow argument materialization through the shared plan. Broader Y/shared-lowering progress is now approximately 50%; direct SAB fallback-removal progress is now approximately 74% by feature track, with current no-fallback unit-file pass rate at 39/64.
