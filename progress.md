...
- [draft] Next active slice is SCI extern/export declaration ordering hardening plus plugin-side fragment cleanup audit.
  - Current verified baseline: full dev-mode direct-SAB no-fallback sweep is 71/71 passing; Y/shared-lowering is approximately 93%; direct SAB fallback-removal is 100% for the tracked unit corpus.
  - Slice progress update (extern/export ordering audit closed as no-repro): a maximal cross-module helper-overlap stress probe (`tests/test_unit_mixed_collections_order.sla`, mixing HashSet + BTreeSet + Vec in one function plus a second function that re-pulls the same set helpers) passes both direct SAB no-fallback and SA-text parity. Host disasm shows every `export_decl $name,fn:N` resolves to a valid function with a coherent following body and no illegal visible `@func(arg)` call targets, so the theoretical filtered std-dep extern/export ordering bug does not reproduce in the tracked corpus. The completed filtered-dep closure sub-slice (`appendDecodedModuleFiltered` same-module direct-call closure + duplicate suppression) already keeps ordering verifier-safe. Added a focused Zig regression (`filtered decoded std deps emit exported helper bodies in original order`) asserting an `exported` helper reached transitively keeps its full body (not a decl-only stub like `external`) and stays in original decoded declaration order relative to the `normal` helper that calls it.
  - Plugin-side fragment cleanup audit conclusion: the plugin's decode-time `renameFragmentText`/`replaceStdMacroPlaceholders*` path is NOT a parallel implementation of SCI's flatten-time `cloneRemappedSymbolText`. SCI's committed embedded remap (`1beccde`) does symbol-ID -> symbol-ID remapping during fragment merge; the plugin path does two things SCI's merge never does: (1) `__frag{N}_` hygiene uniquification across independently-decoded std modules so fragment-internal temps do not alias main codegen or each other, and (2) `__sla_macro_arg_N` caller-arg substitution. These are distinct pipeline stages, so the plugin path stays as a thin decode-time adapter rather than being folded into SCI. No further SCI change is warranted without a concrete failing filtered-dep fixture.
  - This worktree has completed `enum_match`, `spaceship_cmp`, `for_in_protocol`, `generic_for_in_protocol`, `derive_semantics`, `vec_index_assign`, nested Vec field/index assignment, the ready-future async/await subset, and the ready Future/task runtime surface; each focused fixture passed dev-mode direct SAB no-fallback and SA-text parity where applicable.
  - Remaining tracked no-fallback failures: none in `tests/test_unit_*.sla`.
  - Remaining broader hardening: generic SCI fragment naming, pointer-backed struct-update fields, and full async/Future state-machine support beyond the ready-future/task-runtime subset.
  - SCI boundary note: generic std-macro fragment naming belongs in `sci` flatten/encode. Embedded symbol-token/call-body remap is now fixed in SCI for `appendFlattenFragment`. The extern/export ordering audit and plugin-side fragment cleanup audit are now both closed (no-repro / distinct-stage conclusion above). Plugin-side `debug()` structured lowering is retained because it is a proper direct SAB structured path, not a fixture hack.
  - Active slice target: preserve imported extern/export declarations in verifier-safe order, then audit whether `sa_plugin_sla` fragment remap/placeholder handling can consume SCI-owned behavior instead of maintaining a parallel semantic implementation. Current progress: 90% for this slice. Both audit halves are resolved: (1) extern/export ordering closed as no-repro with a maximal cross-module stress fixture + host disasm evidence + new exported-helper-ordering Zig regression, and (2) the plugin-side fragment remap/placeholder path is confirmed a distinct decode-time adapter (hygiene uniquification + caller-arg substitution), not a parallel implementation of SCI's flatten-time symbol-ID remap. Remaining 10%: keep the audit open only as a watch item — a real SCI extern/export ordering change should land only if a concrete failing filtered-dep fixture appears. Broader SCI boundary convergence is about 75% after the embedded token-remap subfeature, the filtered-dep closure sub-slice, and this dual audit closure.
  - `tests/test_unit_vec_index_assign.sla`, `tests/test_unit_field_compare_and_nested_len.sla`, `tests/test_unit_derive_semantics.sla`, `tests/test_unit_generic_for_in_protocol.sla`, `tests/test_unit_async_task_runtime.sla`, `tests/test_unit_struct_update.sla`, `tests/test_unit_sets.sla`, `tests/test_unit_rc_dyn_trait.sla`, `tests/test_unit_var_comprehensive.sla`, `tests/test_unit_enum_match.sla`, `tests/test_unit_spaceship_cmp.sla`, `tests/test_unit_for_in_protocol.sla`, assignment cleanup, `tests/test_unit_pkgjson_codegen.sla`, RefCell struct payload, trait static dispatch, and `/home/vscode/projects/sla_ecs/lib/parallel.sla` are completed regressions and must stay in later host gates.
