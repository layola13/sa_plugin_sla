...
状态：已关闭（2026-07-14）。9 个 strict direct-SAB `&field`/`&*field` runtime signal 11 已修复；两条 strict direct-SAB `--filter` 用例在 local 与 official dev-plugin 入口均通过。
...
2026-07-13 两条均以 code 1 失败，第一条的 SA-text 对照通过 1/1。2026-07-14 复测时两条 local strict SAB filter 均通过 1/1；重新 `SA_PLUGIN_DEV=1 sa plugin install --dev .` 后，official `SA_PLUGIN_DEV=1 sa sla ...` 两条 strict SAB filter 也均通过 1/1。
...
当时仍开放的真正回归：整文件 strict SAB 仍有 9 个用例 `signal 11`，均为 `&field`/`&*field` 借用 smart-pointer 字段后 owner 释放路径冲突，属于 HEAD `1df9edc` 既有问题，需要在共享 call-arg lifecycle plan 中建模 non-owning field-view 与 owner struct cleanup 的关系，不能靠 SA/SAB 各自特判。该子问题已由 2026-07-14 field-view runtime 修复关闭。
...
因此 9 个 field-view runtime signal 11 子问题已关闭，历史 25/25 whole-file 证据恢复。后续 2026-07-14 串行复测还确认 SAB module + `--filter` runtime registration 不再复现 `error: no matching test`。

## 2026-07-14 filter/official closure

串行证据（每次 test/build 前均确认无并发 SLA 构建/测试进程）：

- local strict SAB filter `void call with two deep borrow args releases both owners`：1/1；
- local strict SAB filter `deep owner remains reusable after void call arg cleanup`：1/1；
- local strict SAB whole-file：25/25；
- `SA_PLUGIN_DEV=1 sa plugin install --dev .`：通过；
- `SA_PLUGIN_DEV=1 sa sla help`：通过；
- official dev strict SAB filter `void call with two deep borrow args releases both owners`：1/1；
- official dev strict SAB filter `deep owner remains reusable after void call arg cleanup`：1/1；
- official dev strict SAB whole-file：25/25。

## 期望

- 整文件 local/installed-dev strict SAB 保持 25/25：已满足。
- SA-text 保持 25/25：local 已满足。
- 两条 filter 在 local 和 installed/dev strict SAB 下通过：已满足。
- 后续若该面再回归，修复仍应进入共享 call/lifecycle plan 或 Typed HIR metadata，不能再增加 SA/SAB 各自的释放特判。
