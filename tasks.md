...
- [x] **Direct SAB Output Mainline**
  ...
  - [x] Start the Y-shaped shared lowering-rules path instead of growing independent SA-text and SAB semantic branches.
    ...
    - Progress estimate: the first shared-rules extraction, static-call plan, call-materialization predicate, resolved-call materialization-plan, resolved-call dyn borrow, legacy helper adoption, ordinary function/extern expression-call adoption, remaining SA text legacy method/statement adoption, SAB ordinary static-call adoption, SAB macro static-call adoption, and aggregate ABI layout slices are 100%; the broader Y/shared-lowering track is approximately 48%.
  ...
  - [x] Converge direct-SAB control-flow release scoping and struct-operator lowering with the SA-text emitter so both backends agree on a shared corpus.
    ...
    - Current no-fallback `tests/test_unit_*.sla` sweep is 38/64 passing after the later array-field ABI convergence below. Remaining direct-SAB gaps include async/blank identifier/for-in/enum/set/derive/trait shapes, dyn/refcell/smart-pointer paths, import-contract extern callee resolution, struct update, var comprehensive, and Vec index assignment.
    - Progress estimate: 100% for these three direct-SAB divergence fixes; broader Y/shared-lowering progress is approximately 48%, while direct SAB fallback removal is approximately 73% by feature track after the later array-field ABI convergence.
  - [x] Converge array-typed struct field ABI through shared lowering rules instead of a SAB-only layout branch.
    ...
    - Verified in the current tree: `zig build --summary all`; focused shared ABI Zig unit filter; no-fallback SAB for `test_unit_array_struct_field_cleanup.sla`, `test_unit_struct_field_array_loop.sla`, `test_unit_arrays.sla`, and `test_unit_field_array_cleanup.sla`; SA-text backend for the two fixed fixtures; full current no-fallback `tests/test_unit_*.sla` sweep is 38/64 passing.
    - Progress estimate: 100% for array-typed struct field ABI convergence; broader Y/shared-lowering progress is approximately 48%, and direct SAB fallback removal is approximately 73% by feature track.
  ...
  - [ ] Remove the remaining in-memory SA-compatible fallback from the normal SAB path by replacing std/macro/closure gaps with generic direct SAB lowering or a generic SAB macro representation, not library-name special cases.
    - Progress estimate: approximately 73% overall direct SAB fallback removal; next priority is direct SAB consumption of the remaining shared call/materialization plan kinds and result-destination planning so both emitters reuse lowering decisions.
