...
- [x] **Direct SAB Output Mainline**
  ...
  - [x] Converge direct-SAB control-flow release scoping and struct-operator lowering with the SA-text emitter so both backends agree on a shared corpus.
    ...
    - Current no-fallback `tests/test_unit_*.sla` sweep is 45/64 passing after the later array-field ABI, dyn-borrow plan-consumption, static dispatch resolved-symbol convergence, and Vec index-assignment metadata slices. Remaining direct-SAB gaps include async/blank identifier/for-in/enum/set/derive semantics shapes, rc/dyn/refcell/smart-pointer paths, import-contract extern callee resolution, std import, struct update, and var comprehensive.
    - Progress estimate: 100% for these three direct-SAB divergence fixes; broader Y/shared-lowering progress is approximately 54%, while direct SAB fallback removal is approximately 78% by feature track after the later convergence slices.
  ...
  - [x] Move direct SAB `Vec<T>` index assignment onto std surface metadata instead of a compiler-side Vec ABI branch.
    ...
    - Verified in the current tree: no-fallback SAB and SA-text backend for `test_unit_vec_index_assign.sla`; no-fallback SAB regressions for `test_unit_vec_len_direct.sla`, `test_unit_arrays.sla`, and `/home/vscode/projects/sla_ecs/lib/parallel.sla`; full no-fallback `tests/test_unit_*.sla` sweep is 45/64 passing.
    - Progress estimate: 100% for metadata-driven `Vec<T>` index assignment; broader Y/shared-lowering progress is approximately 54%, and direct SAB fallback removal is approximately 78% by feature track.
  ...
  - [ ] Remove the remaining in-memory SA-compatible fallback from the normal SAB path by replacing std/macro/closure gaps with generic direct SAB lowering or a generic SAB macro representation, not library-name special cases.
    - Progress estimate: approximately 78% overall direct SAB fallback removal; next priority is direct SAB consumption of the remaining shared call/materialization plan kinds and result-destination planning so both emitters reuse lowering decisions.
