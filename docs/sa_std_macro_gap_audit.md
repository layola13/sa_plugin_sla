...
4. Pattern block: Option `while let`, chained `if let`, `let else`, and `match Option<T>` are done for `104`/`105`/`136`/`146`; Result/custom-enum `if let` / `while let` / `let else`, plus match guards over actual tags, are now also covered for the current paths. Covers `30`, `101`, `135`.
...
| Pattern sugar | `30`, `101`, `104`, `105`, `135`, `136`, `146` | Option/Result macros plus enum match lowering exist | `while let Some(...)`, chained `if let Some(...)`, `let Some(...) else`, `match Option<T>`, current Result/custom-enum pattern paths, and real match guards over those tags are done; extend only when later demos require a broader pattern surface. |
...
5. Done for Option `while let` / chained `if let` / `let else` / `match` in `104`, `105`, `136`, and `146`; done for current Result/custom-enum pattern paths and match guards in the compiler and demo `30`.
