...
- Just completed: direct SAB and SA-text `future::pending::<T>()` support for task polling. Shared future runtime call planning now classifies `future::ready` and `future::pending`; type checking accepts pending futures with one generic type argument; both emitters lower the pending constructor through `FUTURE_PENDING_STATE_NEW`. `tests/test_unit_async_pending_task_runtime.sla` passes direct SAB no-fallback and SA-text parity.
...
- Current progress: async pending-future task poll direct SAB 100%. Verified with `zig fmt --check src/lowering_rules.zig src/type_checker.zig src/codegen.zig src/sab_codegen.zig`, `zig build --summary all`, `zig build test --summary all` (68/68), `sa plugin install --dev .`, `SA_PLUGIN_DEV=1 sa sla help`, focused local/host SAB no-fallback plus SA-text parity for `tests/test_unit_async_pending_task_runtime.sla`, local async regression batch, host `/home/vscode/projects/sla_ecs/lib/parallel.sla`, local and host full no-fallback sweeps 76/76, and clean disasm guards for pending/parallel artifacts. Broader Y/shared-lowering about 98%; direct SAB tracked-corpus fallback removal remains 100% (76/76).
