...
- [x] **Direct SAB Output Mainline**
  ...
  - [x] Start the Y-shaped shared lowering-rules path instead of growing independent SA-text and SAB semantic branches.
    ...
    - Progress estimate: the first shared-rules extraction, static-call plan, call-materialization predicate, resolved-call materialization-plan, resolved-call dyn borrow, legacy helper adoption, ordinary function/extern expression-call adoption, remaining SA text legacy method/statement adoption, SAB ordinary static-call adoption, SAB macro static-call adoption, aggregate ABI layout, and SAB dyn-borrow consumption slices are 100%; the broader Y/shared-lowering track is approximately 50%.
  ...
  - [x] Converge direct-SAB control-flow release scoping and struct-operator lowering with the SA-text emitter so both backends agree on a shared corpus.
    ...
    - Current no-fallback `tests/test_unit_*.sla` sweep is 39/64 passing after the later array-field ABI convergence and dyn-borrow plan-consumption slices below. Remaining direct-SAB gaps include async/blank identifier/for-in/enum/set/derive/trait shapes, rc/dyn/refcell/smart-pointer paths, import-contract extern callee resolution, struct update, var comprehensive, and Vec index assignment.
    - Progress estimate: 100% for these three direct-SAB divergence fixes; broader Y/shared-lowering progress is approximately 50%, while direct SAB fallback removal is approximately 74% by feature track after the later convergence slices.
  ...
  - [x] Consume the shared dyn-borrow call-materialization plan in direct SAB.
    ...
    - Verified in the current tree: `zig build --summary all`; no-fallback SAB for `test_unit_dyn_borrow_arg.sla`; SA-text backend for the same fixture; no-fallback SAB regression for `test_unit_overload_add.sla`; full current no-fallback `tests/test_unit_*.sla` sweep is 39/64 passing.
    - Progress estimate: 100% for direct SAB dyn-borrow argument materialization through the shared plan; broader Y/shared-lowering progress is approximately 50%, and direct SAB fallback removal is approximately 74% by feature track.
  ...
  - [ ] Remove the remaining in-memory SA-compatible fallback from the normal SAB path by replacing std/macro/closure gaps with generic direct SAB lowering or a generic SAB macro representation, not library-name special cases.
    - Progress estimate: approximately 74% overall direct SAB fallback removal; next priority is direct SAB consumption of the remaining shared call/materialization plan kinds and result-destination planning so both emitters reuse lowering decisions.
  - [ ] Expand the shared static-call plan into a full call/materialization plan, including parameter-aware auto-borrow, array-to-slice borrow, dyn fat-pointer borrow, temp release policy, and result destination contract shared by SA text and SAB emitters.
    - Progress: shared release policy, auto-borrow predicates, resolved static-call argument materialization, resolved static-call dyn fat-pointer borrow materialization, SA text legacy call array/dyn/materialization adoption, SAB ordinary static-call value/auto-borrow consumption, and SAB macro static-call value/auto-borrow consumption are in place; remaining pieces are explicit result destination planning and direct SAB materialization for array/dyn/copy kinds.
