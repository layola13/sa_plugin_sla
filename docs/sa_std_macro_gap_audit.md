...
| String / `&str` / formatting | `09`, `15`, `35`, `64`, `68`, `69`, `78`, `83`, `87`, `88`, `90`, `99`, `112`, `141`, `147`, `158`, `165`, `167`, `171`, `172`, `184`, `185`, `190` | `STR_LEN`, `STRING_LEN`, `STRING_AS_STR`, `STRING_BUF_*`, `STR_EQ`, `FORMAT_*`, `SLICE_*` | Treat `String` and `&str` separately; typed `String.as_ptr()` plus `String.bytes()/as_bytes()`, borrowed `&str` pointer/byte views, typed `iter().collect<String>()` for current `u8` iterables, and iterable string `join(separator)` for current string-like element sources are now covered through `STRING_BUF_*` / `STR_*`; remaining grouped work is avoiding hardcoded string lengths. |
...
| Mutex / RwLock / mpsc / threads | `61`, `62`, `102`, `121`, `124`, `130`, `184` | `MUTEX_*`, `RWLOCK_*`, `MPSC_*`, existing pthread spawn bridge | `Mutex<i32>::new`, `lock().unwrap()`, guard deref/update, and lexical unlock are done for `102`; `RwLock<i32>::new`, `read().unwrap()`, `write().unwrap()`, guard deref/update, lexical read/write release, and the current captured `Arc<RwLock<i32>>` spawned-reader/join/write/read shape are done for `121`; remaining grouped work is broader resource cleanup and sync shapes beyond the covered Arc/RwLock path. |
| FS / file descriptors | `181`, `182`, plus FFI demos | `FS_OPEN_READ`, `FS_CLOSE`, `FS_READ*`, `FS_METADATA*` | `File::open(...).unwrap()`, `AsRawFd::as_raw_fd`, and compiler-owned lexical `FS_CLOSE` are done for `181`/`182`; remaining grouped work is read/metadata methods and broader file/mmap ownership shapes. |
| Time / sleep | `09`, async demos | `TIME_NOW_*`, `TIME_SLEEP_MS`, `TIME_DURATION_*`, `TIME_INSTANT_*` | `09` now uses explicit `sa_std/time.sla` facade wrappers for `Duration`, `Instant`, `SystemTime`, and `thread::sleep`; continue extending the SLA facade instead of hardcoding concrete std APIs in Zig. |
...
| Box / Rc / raw ownership | `20`, `32`, `51`, `151`, `153`, `154`, `159`, `160`, `178` | `BOX_*`, `RC_*`, `ARC_*`, `MEM_FORGET_U64`, `MANUALLY_DROP_U64_*` | `Box::new`, `Box::into_raw`, unsafe `Box::from_raw`, `mem::forget`, and current `ManuallyDrop<i32>` union-field paths are covered for demos `153`/`154`/`159`/`160`; remaining grouped work is broader Rc/Arc/raw ownership shapes. |
...
| Patterns | `06`, `30`, `39`, `43`, `56`, `60`, `101`, `104`, `105`, `135`, `136`, `146` | `OPTION_BRANCH`, `RESULT_BRANCH`, enum match lowering, `WHILE_LET` | Current Option, Result, and custom-enum `if let` / `while let` / `let else` / `match` paths are covered, including rosetta `104` chained `if let` under strict direct-SAB no-fallback and real match guards over Option/Result/custom-enum tags. SA-text std macro/import pre-scans now also traverse every chained `IfLetCond.value`, so second and later `&& let` values can trigger required Box/Vec/Option/Result/Cell/Atomic/Async/TraitObject imports/helpers. Remaining grouped work is broader pattern surface only when new demos demand it. |
| Traits / supertraits | `07`, `31`, `110`, `163`, `164`, later trait-object demos | `DYN_CALL`, trait-object vtable helpers | Static trait dispatch, borrowed `dyn Trait` calls, object-safety dispatch for `163`, and borrowed `&dyn B` to `&dyn A` supertrait upcasting for `164` are covered through flattened vtable emission and dyn-borrow forwarding; broader owned trait-object/upcast shapes remain future work if later demos demand them. |
...
| `&str` / `String` byte access and pointer access | `112`, `141`, `147`, `158`, `185`, `190` | `STR_AS_BYTES`, `STR_AS_PTR`, `STR_LEN`, `STRING_AS_BYTES`, `STRING_AS_PTR`, `STRING_BUF_AS_PTR`, `STRING_BUF_AS_BYTES`, `STRING_BUF_AS_STR`, `STRING_BUF_INTO_BYTES` | Current lowering now covers borrowed `&str` pointer/byte views via `STR_AS_PTR` / `STR_AS_BYTES`, owned `String.as_ptr()` / `.bytes()` / `.as_bytes()` via `STRING_BUF_AS_PTR` / `STRING_BUF_AS_BYTES`, `u8`-iterable `.collect<String>()` via `STRING_BUF_NEW` / `STRING_BUF_PUSH_BYTE`, and iterable string joins through `STRING_BUF_PUSH_STR` plus `STRING_BUF_AS_STR` for owned separators/elements. Current `STR_AS_PTR` lowering still requires an explicit `@import "sa_std/string.sa"` at the source site; it is not yet auto-surfaced by unrelated imports such as `fmt.sai` / `io/print.sai`. |
| `join` on string slices / CLI arg-like vecs | `90` | `STRING_BUF_NEW`, `STRING_BUF_PUSH_STR`, `STRING_BUF_AS_STR`, `FORMAT_*` | Done for current iterable sources whose elements are string-like (`&str`/string literals or owned `String`) and separators that are borrowed or owned string-like values. |
...
| `Result` combinators | `19`, `50`, `61`, `108`, `173`, `184` | `RESULT_NEW_OK`, `RESULT_NEW_ERR`, `RESULT_UNWRAP`, `RESULT_UNWRAP_OR`, `RESULT_IS_OK`, `RESULT_IS_ERR`, `RESULT_BRANCH` | Current simple `?`, `unwrap`, `unwrap_or`, `is_ok/is_err` paths are covered; the current narrow `catch_unwind` / panic result shape is covered for zero-arg closures that directly call `panic(...)` / `panic_msg(...)`, while broader unwind payload propagation remains future work. |
...
| `Cell` / `RefCell` / guards | `101`, `102`, `106`, `107`, `121`, `124` | `CELL_*`, `REFCELL_*`, `MUTEX_*`, `RWLOCK_*` | `Cell::new/get/set` is done for `106`; `RefCell::new/borrow/borrow_mut` with lexical borrow release is done for `107`; `Mutex<i32>` guard lock/unwrap/deref/update/lexical unlock is done for `102`; `RwLock<i32>` read/write guard lock/unwrap/deref/update/lexical release and current captured `Arc<RwLock<i32>>` thread handoff are done for `121`; remaining grouped work is broader resource cleanup and uncovered RAII shapes. |
| Time / async / await | `09`, `75`, `133`-`136`, `140` | `TIME_*`, `FUTURE_*`, `TASK_*`, `WAKER_*`, plus collection queues | `09` done through `sa_std/time.sla` plus ready-future support; broader async runtime/task/stream/yield/select semantics remain a separate block. |
...
4. Pattern block: Option `while let`, chained `if let`, `let else`, and `match Option<T>` are done for `104`/`105`/`136`/`146`; rosetta `104` chained `if let` is now verified in direct SAB no-fallback as well as SA-text. Result/custom-enum `if let` / `while let` / `let else`, plus match guards over actual tags, are now also covered for the current paths. SA-text std macro/import pre-scan covers all chained `if let` condition values, so pattern-chain values do not miss required std macro families. Covers `30`, `101`, `135`.
5. Interior mutability / guard block: `Cell`, `RefCell`, current `Mutex<i32>` lexical cleanup, current `RwLock<i32>` read/write guard cleanup, and current captured `Arc<RwLock<i32>>` spawned-reader/join/write/read paths are done for `102`, `106`, `107`, and `121`; remaining work is broader resource cleanup plus uncovered RAII/thread shapes. Covers `101`, `102`, `106`, `107`, `121`, `124`.
...
7. IO/raw ownership block: current `File::open(...).unwrap()`, `AsRawFd::as_raw_fd`, lexical `FS_CLOSE`, `Box::into_raw`, unsafe `Box::from_raw`, `mem::forget`, and `ManuallyDrop<i32>` union extraction are done for `153`/`154`/`159`/`160`/`181`/`182`; remaining work is broader pointer `as_ptr/add/read`, and read/metadata file APIs. Covers `112`, `153`, `154`, `159`, `160`, `181`, `182`.
...
- `String.as_ptr()`, `String.as_bytes()`, and `String.bytes()` lower through `sa_std/string.sa` `STRING_BUF_AS_PTR` / `STRING_BUF_AS_BYTES`, compiler-owned cleanup now uses `STRING_BUF_FREE`, owned-`String` view conversions stay on `STRING_BUF_AS_STR`, typed `.collect<String>()` from current `u8` iterables lowers through `STRING_BUF_NEW` / `STRING_BUF_PUSH_BYTE`, and iterable string `.join(separator)` lowers through `STRING_BUF_NEW` / `STRING_BUF_PUSH_STR` plus `STRING_BUF_AS_STR` for owned separators/elements without depending on `sa_std/string_format.sa` being imported transitively.
...
- `RwLock<i32>::new`, `read().unwrap()`, `write().unwrap()`, guard deref/read/update, compiler-owned lexical read/write release, and the current captured `Arc<RwLock<i32>>` spawned-reader/join/write/read shape lower through explicit `sa_std/core/arc.sa`, `sa_std/sync/rwlock.sa`, `sa_std/thread.sa`, and `sa_std/core/result.sa` imports for `121`.
- `File::open(...).unwrap()`, `file.as_raw_fd()`, and compiler-owned lexical close lower through explicit `sa_std/fs.sa` plus `sa_std/core/result.sa` imports for `181` and `182`.
- `Duration::from_millis/as_millis`, `Instant::now/elapsed`, `SystemTime::now/unix_epoch/duration_since(...).unwrap()`, and `thread::sleep(Duration)` are available through explicit `sa_std/time.sla` facade imports for current `09` paths; Zig only provides generic `.sla` import expansion and generic associated-call lowering.
- `Box::into_raw` and unsafe `Box::from_raw` lower through explicit `sa_std/core/box.sa` imports and the existing `BOX_INTO_RAW` / `BOX_FROM_RAW` macros for `153` and `154`, with `into_raw` consuming the Box binding and `from_raw` restoring Box ownership.
- `mem::forget` lowers through explicit `sa_std/mem.sa` imports and `MEM_FORGET_U64` for current 8-byte owned values, consuming the root binding so forgotten `Box` values are not auto-released and later use is rejected.
- `ManuallyDrop::new` / `ManuallyDrop::into_inner` lower through explicit `sa_std/mem.sa` imports and `MANUALLY_DROP_U64_*` macros for current `ManuallyDrop<i32>` paths, including union field in-place initialization and unsafe union field extraction for `160`.
...
- Static trait dispatch, borrowed trait-object calls for current `dyn Trait` paths, `trait B: A` declaration syntax, object-safety dispatch for `163`, and borrowed supertrait upcasting for `164` lower through flattened supertrait vtables and dyn-borrow forwarding.
- `@no_mangle pub extern "C" fn` definitions lower to raw unmangled SA symbols, pointer-returning extern calls now type-check as real pointer values, and terminating `unsafe { ... }` expression blocks preserve their function-tail behavior for current `111` / `185` paths.
...
| `Cell` / `RefCell` / `Mutex` / `RwLock` | `101`, `102`, `106`, `107`, `121`, `124` | `CELL_*`, `REFCELL_*`, `MUTEX_*`, `RWLOCK_*` macros exist | `Cell::new/get/set` is done for i32 `106`; `RefCell::new/borrow/borrow_mut` with lexical borrow release is done for integer `107`; `Mutex<i32>` lock guard cleanup is done for `102`; `RwLock<i32>` read/write guard cleanup is done for `121`; next add broader RAII/resource semantics needed by `101` and `124`. |
| Pattern sugar | `30`, `101`, `104`, `105`, `135`, `136`, `146` | Option/Result macros plus enum match lowering exist | `while let Some(...)`, chained `if let Some(...)` including rosetta `104` direct-SAB no-fallback, `let Some(...) else`, `match Option<T>`, current Result/custom-enum pattern paths, and real match guards over those tags are done; extend only when later demos require a broader pattern surface. |
...
| File RAII | `181_file_descriptor_raii`, `182_mmap_memory_mapping` | `FS_OPEN_READ`, `FS_CLOSE`, read/metadata macros exist | Done for current `File::open(...).unwrap()`, `as_raw_fd()`, and lexical close cleanup; extend with read/metadata file methods when demos require them. |
| `Box` / `mem` raw APIs | `153_box_into_raw`, `154_box_from_raw`, `159_mem_forget_leak`, `160_manually_drop_union` | `BOX_*`, `MEM_FORGET_U64`, and `MANUALLY_DROP_U64_*` low-level macros exist | Done for `Box::into_raw`, unsafe `Box::from_raw`, pointer ownership transfer, `mem::forget`, and current `ManuallyDrop<i32>` union extraction paths. |
| `String` collection | `190_base64_encode_simd` | `STRING_BUF_*`, `FORMAT_*`, vec/string macros exist | Done for current `u8` iterable sources via typed `.collect<String>()`, and current iterable string joins via `.join(separator)`; keep future work focused on any non-`u8` collection shapes only if demos require them. |
...
| Dynamic supertrait objects / upcasting | `110`, `163`, `164` | Current borrowed dyn dispatch and borrowed supertrait upcasting are covered for demos `163`/`164`; remaining future work is owned trait-object storage, broader coercion sites, and later trait-object shapes beyond the current rosetta pressure. |
| Thread-local static | `124_thread_local_storage` | Current demo `124` uses an explicit accepted subset: a `ThreadLocalSlot { thread_id, value: Cell<i32> }` state object with `Cell` get/set. Real `thread_local!` syntax and true per-thread static storage remain future work. |
...
5. Done for Option `while let` / chained `if let` / `let else` / `match` in `104`, `105`, `136`, and `146`; rosetta `104` chained `if let` is verified through direct SAB no-fallback; done for current Result/custom-enum pattern paths and match guards in the compiler and demo `30`.
