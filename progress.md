...
- [draft] Next active slice is SCI extern/export declaration ordering hardening plus plugin-side fragment cleanup audit.
  - Current verified baseline: full dev-mode direct-SAB no-fallback sweep is 69/69 passing; Y/shared-lowering is approximately 92%; direct SAB fallback-removal is 100% for the tracked unit corpus.
...
- [done] Filtered std-dep injection now preserves same-module helper closure without duplicate selected helpers.
  ...
  - Feature completion: filtered std-dep closure sub-slice 100%; broader SCI boundary convergence about 60% -> 65%; Y/shared-lowering remains approximately 92%; direct SAB fallback-removal for tracked corpus remains 100%; no-fallback sweep 69/69; committed in `0a750b0`.
