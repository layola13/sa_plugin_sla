...
| `HashMap` / `BTreeMap` / sets | `53`, `63`, `81`; set-like pressure later | `MAP_*`, `BTREE_MAP_*`, `SET_*`, `BTREE_SET_*` | `HashMap` baseline exists; implement `BTreeMap::new/insert/get/index/len/free` before `81`, and keep map/set `.len()` receiver-typed. |
...
| Mutex / RwLock / mpsc / threads | `61`, `62`, `102`, `121`, `124`, `130`, `184` | `MUTEX_*`, `RWLOCK_*`, `MPSC_*`, existing pthread spawn bridge | Add guard objects with lexical unlock, `lock().unwrap()`, `RwLock` guards, and preserve existing `mpsc` result wrapping. |
...
| `HashMap` / `BTreeMap` | `53`, `63`, `81` | `MAP_*`, `BTREE_MAP_*`, `OPTION_*` | `HashMap` current string-key/i32-value path is done; `BTreeMap` block is still pending. |
| `Cell` / `RefCell` / guards | `101`, `102`, `106`, `107`, `124` | `CELL_*`, `REFCELL_*`, `MUTEX_*`, `RWLOCK_*` | `Cell::new/get/set` is done for `106`; `RefCell::new/borrow/borrow_mut` with lexical borrow release is done for `107`; remaining grouped work is `Mutex`/`RwLock` guards and broader resource cleanup. |
...
3. Collection block B: `BTreeMap::new/insert/get/index/len/free`. Covers `81` and prevents separate map special cases.
...
5. Interior mutability / guard block: `Cell`, `RefCell`, `Mutex` with lexical cleanup. Covers `101`, `102`, `106`, `107`, `124`.
...
| `BTreeMap` | `81_kv_store` | `BTREE_MAP_NEW`, `INSERT`, `GET`, `LEN`, `FREE` exist | Add `BTreeMap::new`, `insert`, indexing/get, len, cleanup. |
...
| `Cell` / `RefCell` | `101`, `106`, `107`, `124` | `CELL_*`, `REFCELL_*` macros exist | `Cell::new/get/set` is done for i32 `106`; `RefCell::new/borrow/borrow_mut` with lexical borrow release is done for integer `107`; next add broader RAII/guard semantics needed by `101`, `102`, and `124`. |
...
| Area | Rust demo pressure | `sa_std` macro status | Missing Sla lowering |
| --- | --- | --- | --- |
| `Mutex` guard | `102_raii_guard` | `MUTEX_NEW`, `LOCK`, `UNLOCK` exist | Add `Mutex::new`, `lock().unwrap()`, guard deref/update, lexical guard release without `drop` keyword. |
...
| Unsafe/FFI/raw pointers | `112`-`120`, `153`-`160`, `181`-`187` | `111` extern C ABI definition/call is done; the remaining low-level demos should still be tackled as a separate FFI/raw pointer phase. |
...
3. Implement `BTreeMap` lowering as one collection block before `81`.
