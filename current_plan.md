...
- Latest completed slices (verified with dev-mode SAB no-fallback + SA-text parity where applicable): scalar and pointer-backed `struct_update`, formatted `println` direct SAB lowering, `enum_match`, `spaceship_cmp`, `for_in_protocol`, `generic_for_in_protocol`, `derive_semantics`, `vec_index_assign`/nested Vec field assignment, the ready-future async/await subset, the ready Future/task runtime surface, and direct `block_on` over ready futures with fallible std-time externs.
- Current full dev-mode direct SAB no-fallback sweep: 74/74 passing (added `tests/test_unit_async_block_on_direct.sla`).
- Current global estimates: Y/shared-lowering about 96%; direct SAB fallback-removal is 100% for the tracked unit corpus, with corpus pass rate now 74/74.
- Current feature report: `async block_on ready-future direct SAB 100%; no-fallback sweep is 74/74; commit pending in this slice`.
...
- Just completed: direct SAB `block_on` over ready futures. Shared ABI rules now classify `future<T>` as pointer-backed, and `.sai` contract parsing preserves fallible extern returns for direct SAB signatures. `tests/test_unit_async_block_on_direct.sla` covers a future parameter, Box/Pin ready polling, and std time sleep through `sa_time_sleep_ns -> i32!`; rosetta `09_async_await` now passes no-fallback direct SAB and SA-text parity.
...
- Current progress: async `block_on` ready-future direct SAB 100%. Verified with `zig fmt --check src/contract_parser.zig src/lowering_rules.zig src/sab_codegen.zig`, `zig build --summary all`, `zig build test --summary all` (65/65), `sa plugin install --dev .`, `SA_PLUGIN_DEV=1 sa sla help`, focused local and host SAB no-fallback plus SA-text parity for `tests/test_unit_async_block_on_direct.sla` and rosetta `09_async_await`, host async rosetta guards `75`, `133`, `134`, `135`, `139`, and `140`, host `/home/vscode/projects/sla_ecs/lib/parallel.sla`, local and host full no-fallback sweeps 74/74, disasm guard clean for async block-on/rosetta/parallel artifacts, `sa_time_sleep_ns` preserved as a fallible `i32!` extern, and `git diff --check`. Broader Y/shared-lowering about 96%; direct SAB tracked-corpus fallback removal remains 100% (74/74).
