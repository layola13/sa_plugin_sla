...
- [x] **Direct SAB Output Mainline**
  ...
  - [x] Start the Y-shaped shared lowering-rules path instead of growing independent SA-text and SAB semantic branches.
    ...
    - Progress estimate: the first shared-rules extraction, static-call plan, call-materialization predicate, resolved-call materialization-plan, resolved-call dyn borrow, legacy helper adoption, ordinary function/extern expression-call adoption, remaining SA text legacy method/statement adoption, SAB ordinary static-call adoption, and SAB macro static-call adoption slices are 100%; the broader Y/shared-lowering track is approximately 38%.
  ...
  - [x] Converge direct-SAB control-flow release scoping and struct-operator lowering with the SA-text emitter so both backends agree on a shared corpus.
    ...
    - Current no-fallback `tests/test_unit_*.sla` sweep is 36/64 passing. Remaining direct-SAB gaps include array-in-struct ABI, async/blank identifier/for-in/enum/set/derive/trait shapes, dyn/refcell/smart-pointer paths, import-contract extern callee resolution, struct update, var comprehensive, and Vec index assignment.
    - Progress estimate: 100% for these three direct-SAB divergence fixes; broader Y/shared-lowering progress is approximately 45%, while direct SAB fallback removal remains approximately 72% by feature track.
    - Next Y target: array-typed struct fields are represented as inline bytes by SAB `fieldLayout`/`typeSize` but as heap pointers by SA-text; converge this ABI so arrays-in-structs lower identically.
