...
- [x] **Direct SAB Output Mainline**
  ...
  - [x] Converge direct-SAB control-flow release scoping and struct-operator lowering with the SA-text emitter so both backends agree on a shared corpus.
    ...
    - Current no-fallback `tests/test_unit_*.sla` sweep is 47/64 passing after the later array-field ABI, dyn-borrow plan-consumption, static dispatch resolved-symbol convergence, Vec index-assignment metadata, fixed-array destructuring/std-slice-index, and Cell std-surface slices. Remaining direct-SAB gaps include async/for-in/enum/set/derive semantics shapes, borrow-temp release ordering, rc/dyn/refcell/smart-pointer paths, import-contract extern callee resolution, std import, struct update, and var comprehensive.
    - Progress estimate: 100% for these three direct-SAB divergence fixes; broader Y/shared-lowering progress is approximately 56%, while direct SAB fallback removal is approximately 80% by feature track after the later convergence slices.
  ...
  - [ ] Remove the remaining in-memory SA-compatible fallback from the normal SAB path by replacing std/macro/closure gaps with generic direct SAB lowering or a generic SAB macro representation, not library-name special cases.
    - Progress estimate: approximately 80% overall direct SAB fallback removal; next priority is direct SAB consumption of the remaining shared call/materialization plan kinds and result-destination planning so both emitters reuse lowering decisions.
