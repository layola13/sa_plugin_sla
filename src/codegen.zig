...
pub const Codegen = struct {
    ...
    fn derivedComponentMetadataValue(self: *Codegen, target_name: []const u8, func_name: []const u8) ?i64 {
        ...
        if (std.mem.eql(u8, func_name, "component_type_id")) return componentTypeIdForName(target_name);
        if (std.mem.eql(u8, func_name, "component_storage_kind")) return 0;
        return null;
    }
            ...
            .call_expr => |call| {
                ...
                if (call.associated_target) |target| {
                    ...
                    if (self.tc.funcs.contains(method_key)) {
                        ...
                        self.out.writer().print(")\n", .{}) catch return CodegenError.CodegenError;
                        for (call.args, arg_regs.items) |arg, arg_reg| {
                            if (callArgNeedsRelease(arg) or self.generatedFnPtrIdentifierArg(arg)) try self.emitRelease(arg_reg);
                        }
                        return reg;
                    }
                }
                ...
                if (self.tc.extern_funcs.contains(call.func_name) or self.tc.funcs.contains(call.func_name)) {
                    ...
                    for (call.args, arg_regs.items) |arg, arg_reg| {
                        if (self.tc.dyn_borrow_args.contains(arg)) continue;
                        if ((callArgNeedsRelease(arg) or self.generatedFnPtrIdentifierArg(arg)) and !std.mem.eql(u8, call.func_name, "sum")) {
